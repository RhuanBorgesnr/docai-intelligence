...
class ChatStreamAPIView(APIView):
    ...
    Each chunk is sent as it's generated by the LLM for real-time typing effect.
    """
    permission_classes = [IsAuthenticated]
    throttle_classes = [ScopedRateThrottle]
    throttle_scope = 'ai'
    ...
    def post(self, request):
        ...
        if not allowed_ids:
            return Response({"error": "Nenhum documento válido selecionado."}, status=status.HTTP_400_BAD_REQUEST)

        # Build context for RAG
        from ai.rag import _build_context
        context = _build_context(allowed_ids, question)

        # Stream response
        from ai.groq_client import chat_with_groq_stream, is_groq_enabled
        ...
        def event_stream():
            ...
            for chunk in chat_with_groq_stream(context, question):
